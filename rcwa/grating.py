from rcwa import Layer, Slicer, Crystal, Material
import numpy as np
...
class RectangularGrating(Grating):
    ...
    def __init__(self, period: float = 1, er: float = 2, ur: float = 1, n: Union[float, None] = None,
        ...
        if groove_width > period:
            raise ValueError(f'Groove width {groove_width} must be smaller than period {period}')
        ...
        offset_fraction = offset / period

        er_data, ur_data = self._er_data(
            er=er, ur=ur, n=n, er_void=er_void, ur_void=ur_void, n_void=n_void,
            Nx=nx, groove_fraction=groove_fraction, offset_fraction=offset_fraction)
    ...
    def _er_data(self, er: float = 2, er_void: float = 1, ur: float = 1, ur_void: float = 1,
        ...
        '''
        if n is not None:
            er_data = self._er_data_single(np.square(n_void), np.square(n), Nx, groove_fraction, offset_fraction)
            ur_data = np.ones(er_data.shape)
        else:
            er_data = self._er_data_single(er_void, er, Nx, groove_fraction, offset_fraction)
            ur_data = self._er_data_single(ur_void, ur, Nx, groove_fraction, offset_fraction)

        return er_data, ur_data

    def _er_data_single(self, val1: float, val2: float, Nx: int, switch_fraction: float, offset_fraction) -> ArrayLike:
