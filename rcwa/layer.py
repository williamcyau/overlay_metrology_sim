...
class Layer(MatrixCalculator):
    ...
    def __init__(self, er: complex = 1.0, ur: complex = 1.0, thickness: complex = 0.0, n: Union[complex, None] = None,
        ...
        if crystal is None:
            ...
            self.er_homogenous_scalar = er
            self.ur_homogenous_scalar = ur
        else:
...
class LayerStack:
    ...
    :param internal_layers: Layer objects, starting with the top-most layer (reflection region) and ending with the bottom-most region (substrate)
    ...
    def total_thickness(self):
        '''
        Note: For the sake of proper visualization, each "nothing" layer (i.e., thickness = 0) is treated as having a thickness of 0.02.
        '''
        z_tot = 0
    ...
    def er_ur_geometry(self, Nx):
        ...
        for l in self.all_layers:
            ...
            if z == 0:
                z = 0.02
            if l.homogenous:
                er_matrix = np.ones((int(Nz*z/self.total_thickness), Nx))*l.er_homogenous_scalar
                ur_matrix = np.ones((int(Nz*z/self.total_thickness), Nx))*l.ur_homogenous_scalar
            else:
                try:
                    er_matrix = np.broadcast_to(l.crystal.permittivityCellData[np.newaxis, :], (round(Nz*z/self.total_thickness), Nx))
                    ur_matrix = np.broadcast_to(l.crystal.permeabilityCellData[np.newaxis, :], (round(Nz*z/self.total_thickness), Nx))
                except ValueError:
                    print(f"ValueError: input Nx = {Nx} does not match layer Nx = {len(l.crystal.permittivityCellData)}")
        ...
        return er_matrix_global, ur_matrix_global
