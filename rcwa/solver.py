...
class Solver:
    ...
    def reflectedElectricComponents_1D(self):
        incidentFieldHarmonics = s_incident(self.source, self.n_harmonics)

        ReflectionRegionHarmonicsCoefficients = np.linalg.inv(self.WReflectionRegion) @ incidentFieldHarmonics
        ReflectedHarmonicsCoefficients = self.SGlobal[0, 0] @ ReflectionRegionHarmonicsCoefficients
        ...
        ReflectedHarmonicsCmplxAmp_Z = - np.linalg.inv(self.KzReflectionRegion) @ (self.Kx @ ReflectedHarmonicsCmplxAmp_X + self.Ky @ ReflectedHarmonicsCmplxAmp_Y)
    ...
    def kx_vector(self):
        '''
        Returns a vector of kx, i.e., the diagonal of matrix Kx
        '''
        return np.diag(self.Kx)

    @property
    def ky_vector(self):
        '''
        Returns a vector of ky, i.e., the diagonal of matrix Ky
        '''
        return np.diag(self.Ky)
    ...
    def compute_global_s_matrix_to_i_layer(self, layer_idx):
        '''
        Note: layer_idx starts from the the first meaningful layer, i.e., not the reflection region
        '''
        SGlobal_to_i = self.SReflection
        for i in range(layer_idx):
            SGlobal_to_i = redheffer_product(SGlobal_to_i, self.Si[i])
        return SGlobal_to_i
    
    def compute_external_coefficients_i_layer(self, layer_idx):
        '''
        Note: layer_idx starts from the the first meaningful layer, i.e., not the reflection region
        '''

        # to compute the ith layer, we need the S-matrix until the i-1 layer
        SGlobal_to_i_minus_1 = self.compute_global_s_matrix_to_i_layer(layer_idx - 1)
        ...
        ReflectedHarmonicsCoefficients = self.SGlobal[0, 0] @ ReflectionRegionHarmonicsCoefficients
        Coefficients_prev_i_ref = np.linalg.inv(SGlobal_to_i_minus_1[0, 1]) @ (ReflectedHarmonicsCoefficients - SGlobal_to_i_minus_1[0, 0] @ incidentFieldHarmonics)
        Coefficients_prev_i_trn = SGlobal_to_i_minus_1[1, 0] @ incidentFieldHarmonics + SGlobal_to_i_minus_1[1, 1] @ Coefficients_prev_i_ref

        return Coefficients_prev_i_ref, Coefficients_prev_i_trn

    def compute_internal_coefficients_i_layer(self, layer_idx):
        Coefficients_prev_i_ref, Coefficients_prev_i_trn = self.compute_external_coefficients_i_layer(layer_idx)
        Coefficients_prev_i = np.concatenate((Coefficients_prev_i_trn, Coefficients_prev_i_ref))
        prev_i_layer = self.layer_stack.internal_layers[layer_idx - 1]
        prev_i_layer_W = prev_i_layer.W_matrix
        prev_i_layer_V = prev_i_layer.V_matrix

        prev_i_layer_matrix = np.block([[prev_i_layer_W, prev_i_layer_W],[-prev_i_layer_V, prev_i_layer_V]])

        i_layer = self.layer_stack.internal_layers[layer_idx]
        i_layer_W = i_layer.W_matrix
        i_layer_V = i_layer.V_matrix

        i_layer_matrix = np.block([[i_layer_W, i_layer_W],[-i_layer_V, i_layer_V]])

        Coefficients_i = np.linalg.inv(i_layer_matrix) @ prev_i_layer_matrix @ Coefficients_prev_i
        return Coefficients_i
